...
local fusion_rtu   = require("rtu.dev.fusion_rtu")
    ...
    if unit.name == iface then
        ...
        if unit.type == RTU_UNIT_TYPE.VIRTUAL then
            ...
            elseif type == "fusionReactorController" or type == "fusionReactorPort" then
                ...
                if unit.reactor < 1 or unit.reactor > 4 then fail(util.c("fusion reactor '", unit.name, "' cannot init, not assigned to a valid unit")) end
                if unit.index ~= 1 then fail(util.c("fusion reactor '", unit.name, "' cannot init, invalid index provided")) end

                unit.type = RTU_UNIT_TYPE.FUSION
        ...
        elseif unit.type == RTU_UNIT_TYPE.SPS then
            ...
            unit.formed = util.trinary(faulted, false, nil)
        elseif unit.type == RTU_UNIT_TYPE.FUSION then
            unit.rtu, faulted = fusion_rtu.new(device)
            unit.formed = util.trinary(faulted, false, nil)
