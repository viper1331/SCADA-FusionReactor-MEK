...
return function(config, __shared_memory)
    ...
    for i = 1, #rtu_devices do
        ...
        local function validate_index(min, max)
            if (not util.is_int(index)) or (index < min) or (max ~= nil and index > max) then
        ...
        elseif type == "fusionReactorController" or type == "fusionReactorPort" then
            ...
            rtu_type = RTU_UNIT_TYPE.FUSION
            rtu_iface, faulted = fusion_rtu.new(device)
            is_multiblock = true
            formed = device.isFormed()

            if formed == ppm.ACCESS_FAULT then
                println_ts(util.c("uinit> failed to check if '", name, "' is formed"))
                log.warning(util.c("uinit> failed to check if '", name, "' is a formed fusion reactor multiblock"))
            end
