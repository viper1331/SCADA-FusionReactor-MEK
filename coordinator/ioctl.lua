...
local FUS_STATE = types.FUSION_STATE
...
---@diagnostic disable-next-line: missing-fields
            ...
            fusion_ps_tbl = {},     ---@type psil[]
            fusion_data_tbl = {},   ---@type fusion_session_db[]
        ...
        -- fusion pages subscribe to fusion #1, even before we know if one exists
        table.insert(entry.fusion_ps_tbl, psil.create())

        entry.num_boilers = #entry.boiler_data_tbl
        entry.num_turbines = #entry.turbine_data_tbl

        entry.unit_ps.publish("has_fusion", false)
        entry.unit_ps.publish("fusion_computed_status", FUS_STATE.OFFLINE)
...
---@param id integer
---@param entry table
---@param data_tbl (imatrix_session_db|sps_session_db|dynamicv_session_db|turbinev_session_db|boilerv_session_db|fusion_session_db)[]
---@param ps_tbl psil[]
...
function ioctl.record_unit_builds(builds)
    ...
    for id, build in pairs(builds) do
        ...
        elseif type(unit) ~= "table" then
            ...
            -- fusion reactor builds
            if type(build.fusion) == "table" then
                for f_id, fusion in pairs(build.fusion) do
                    if not _record_multiblock_build(f_id, fusion, unit.fusion_data_tbl, unit.fusion_ps_tbl, true) then
                        log.debug(util.c(log_header, "invalid fusion reactor id ", f_id))
                        valid = false
                    end
                end
            end
            ...
            unit.unit_ps.publish("has_fusion", type(unit.fusion_data_tbl[1]) == "table")
        end
    end
...
---@param entry any
---@param data imatrix_session_db|sps_session_db|dynamicv_session_db|turbinev_session_db|boilerv_session_db|fusion_session_db
---@param ps psil
        ...
        for i = 1, #statuses do
            ...
            if type(status) ~= "table" or #status ~= 6 then
                ...
                if type(rtu_statuses) == "table" then
                    ...
                    -- fusion reactor statuses
                    if type(rtu_statuses.fusion) == "table" then
                        computed_status = FUS_STATE.OFFLINE

                        for id = 1, #unit.fusion_ps_tbl do
                            if rtu_statuses.fusion[id] == nil then
                                unit.fusion_ps_tbl[id].publish("computed_status", computed_status)
                            end
                        end

                        for id, fusion in pairs(rtu_statuses.fusion) do
                            if type(unit.fusion_data_tbl[id]) == "table" then
                                local data = unit.fusion_data_tbl[id]
                                local ps   = unit.fusion_ps_tbl[id]

                                local rtu_faulted = _record_multiblock_status(fusion, data, ps)

                                if rtu_faulted then
                                    computed_status = FUS_STATE.FAULT
                                elseif data.formed then
                                    local active = data.state.ignited or
                                        ((data.state.prod_rate or 0) > 0) or
                                        ((data.state.passive_generation or 0) > 0)
                                    computed_status = util.trinary(active, FUS_STATE.ACTIVE, FUS_STATE.IDLE)
                                else computed_status = FUS_STATE.UNFORMED end

                                ps.publish("computed_status", computed_status)

                                if id == 1 then
                                    unit.unit_ps.publish("fusion_computed_status", computed_status)
                                end
                            else
                                log.debug(util.c(log_header, "invalid fusion reactor id ", id))
                                valid = false
                            end
                        end
                    else
                        log.debug(log_header .. "fusion reactor list not a table")
                        valid = false
                    end
                    ...
                    if type(unit.fusion_data_tbl[1]) ~= "table" then
                        unit.unit_ps.publish("fusion_computed_status", FUS_STATE.OFFLINE)
                    end
                else
