...
local fusion_view  = require("coordinator.ui.components.fusion")
...
local function make(parent, x, y, unit)
    ...
    local reactor_pane = Div{parent=root}
    local fusion_pane = Div{parent=root}

    reactor_view(reactor_pane, 1, 3, unit.unit_ps)
    fusion_view(fusion_pane, 1, 3, unit.fusion_ps_tbl[1])

    local reactor_mode = MultiPane{parent=root,panes={reactor_pane,fusion_pane}}

    unit.unit_ps.subscribe("has_fusion", function (has_fusion)
        reactor_mode.set_value(util.trinary(has_fusion, 2, 1))
    end)
