...
local fusion        = require("pocket.ui.pages.unit_fusion")
...
local function new_view(root)
    ...
    local function set_sidebar(id)
        ...
        local has_fusion = unit.unit_ps.get("has_fusion") == true
        local fusion_pages = nav_links[id].fusion or {}

        if has_fusion then
            local multi = #fusion_pages > 1

            for i = 1, #fusion_pages do
                local label = util.trinary(multi, "F-" .. i, "FUS")
                table.insert(list, { label = label, color = core.cpair(colors.black, colors.lightGray), callback = fusion_pages[i] })
            end
        end
    ...
    local function load()
        ...
        for i = 1, db.facility.num_units do
            ...
            local fission_type = util.trinary(unit.num_boilers > 0, "Sodium Cooled Reactor", "Boiling Water Reactor")
            local unit_type = TextBox{parent=u_div,y=3,text=fission_type,alignment=ALIGN.CENTER,fg_bg=cpair(colors.gray,colors.black)}
            ...
            for f = 1, #unit.fusion_ps_tbl do
                local fzr = IconIndicator{parent=u_div,label="Fusion "..f,states=basic_states}
                fzr.register(unit.fusion_ps_tbl[f], "FusionStatus", fzr.update)
                table.insert(fusion_icons, fzr)
            end

            u_ps.subscribe("has_fusion", function (has_fusion)
                for _, icon in ipairs(fusion_icons) do
                    if has_fusion then icon.show() else icon.hide() end
                end

                if has_fusion then
                    unit_type.set_value("Fusion Reactor")
                else
                    unit_type.set_value(fission_type)
                end

                if active_unit == i and nav_links[i].alarm ~= nil then set_sidebar(i) end
            end)
            ...
            local fus_pane = Div{parent=page_div}
            nav_links[i].fusion = {}
