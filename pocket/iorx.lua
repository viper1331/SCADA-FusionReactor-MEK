...
function iorx.record_unit_data(data)
    ...
    local fusion_data = {}
    local tank_data_idx = 10
    local rate_idx = 11
    local stable_idx = 12
    local has_fusion_payload = false

    -- compatibility: API_GET_UNIT gained fusion data at index 10
    if type(data[13]) == "boolean" then
        has_fusion_payload = true
    ...
    unit.fusion_data_tbl = fusion_data

    for id = 1, #unit.fusion_ps_tbl do
        if unit.fusion_ps_tbl[id] == nil then
            unit.fusion_ps_tbl[id] = psil.create()
        end

        local ps     = unit.fusion_ps_tbl[id]
        local fusion = unit.fusion_data_tbl[id]
        local c_stat = util.trinary(has_fusion_payload, comp_statuses[next_c_stat] or FUS_STATE.OFFLINE, FUS_STATE.OFFLINE)

        local fusion_status = 1

        if c_stat ~= FUS_STATE.OFFLINE then
            if c_stat == FUS_STATE.FAULT then
                fusion_status = 3
            elseif type(fusion) == "table" and fusion.formed then
                fusion_status = 4
            else
                fusion_status = 2
            end
            ...
            if type(fusion) == "table" then
                _record_multiblock_status(c_stat == FUS_STATE.FAULT, fusion, ps)
            end
        end

        ps.publish("FusionStatus", fusion_status)
        ps.publish("FusionStateStatus", c_stat)

        if has_fusion_payload then
            next_c_stat = next_c_stat + 1
        end
    end

    u_ps.publish("has_fusion", type(unit.fusion_data_tbl[1]) == "table")

    unit.tank_data_tbl = data[tank_data_idx]
    ...
    unit.last_rate_change_ms = data[rate_idx]
    unit.turbine_flow_stable = data[stable_idx]
