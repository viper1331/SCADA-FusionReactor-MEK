...
return function (args)
    ...
    local text = util.localize(args.text)
    local active_text = util.trinary(type(args.active_text) == "string", util.localize(args.active_text), nil)
    local text_width = string.len(text)

    if type(active_text) == "string" then
        text_width = math.max(text_width, string.len(active_text))
    end
    ...
    function e.redraw()
        ...
        e.w_write(util.trinary(e.value and active_text, active_text, text))
    end
