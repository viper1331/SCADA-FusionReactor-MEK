...
return function (args)
    ...
    for i = 1, #args.options do
        options[i] = util.localize(args.options[i])
    end

    element.assert((args.rows * args.columns) >= #options, "rows x columns size insufficient for provided number of options")
    ...
    for col = 1, args.columns do
        ...
        for row = 1, args.rows do
            local len = string.len(options[next_idx])
            if len > max_width then max_width = len end
            if row > max_rows then max_rows = row end

            table.insert(array[col], { text = options[next_idx], id = next_idx, x_1 = 1 + total_width, x_2 = 2 + total_width + len })

            next_idx = next_idx + 1
            if next_idx > #options then break end
        end
    ...
    function e.set_value(val)
        if type(val) == "number" and val > 0 and val <= #options then
            e.value = val
            e.redraw()
        end
    end
