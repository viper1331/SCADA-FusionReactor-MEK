...
return function (args)
    ...
    e.value = args.value
    local label = util.localize(args.label)
    local unit_label = util.localize(args.unit)
    ...
    function e.on_update(value)
        ...
        local data_str, unit = util.power_format(value, unit_label, false, args.format)
    ...
    function e.redraw()
        ...
        data_start = string.len(label) + 2
        if string.len(label) == 0 then data_start = 1 end
