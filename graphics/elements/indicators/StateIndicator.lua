...
return function (args)
    ...
    for i = 1, #args.states do
        ...
        local text = util.localize(state_def.text)

        if string.len(text) > args.width then
            args.width = string.len(text)
        end

        table.insert(state_blit_cmds, {
            text = util.pad(text, args.width),
            fgd = string.rep(state_def.color.blit_fgd, args.width),
            bkg = string.rep(state_def.color.blit_bkg, args.width)
        })
    end
