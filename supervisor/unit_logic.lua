...
function logic.update_status_text(self)
    ...
    if is_active(self.alarms.ContainmentBreach) then
        ...
        if self.plc_i ~= nil and self.plc_i.get_rps().force_dis then
            self.status_text = { "REACTEUR FORCE DESACTIVE", "fusion du coeur evitee" }
        else
            self.status_text = { "FUSION DU COEUR", "reacteur detruit" }
        end
    elseif is_active(self.alarms.CriticalDamage) then
        -- so much for it being a "routine turbin' trip"...
        self.status_text = { "FUSION IMMINENTE", "evacuez l installation immediatement" }
    elseif is_active(self.alarms.ReactorDamage) then
        -- attempt to determine when a chance of a meltdown will occur
        self.status_text[1] = "CONFINEMENT ENDOMMAGE"
        if self.plc_cache.damage >= 100 then
            self.status_text[2] = "degats critiques"
        elseif (self.plc_cache.damage < self.damage_last) or ((self.plc_cache.damage - self.damage_initial) < 0) then
            ...
            self.status_text = { "CONFINEMENT ENDOMMAGE", "niveau de degats en baisse..." }
        ...
        elseif (not self.damage_decreasing) or (self.plc_cache.damage > self.damage_last) then
            ...
            if (self.plc_cache.damage - self.damage_initial) > 0 then
                ...
                self.status_text[2] = util.c("degats critiques dans ", util.sprintf("%.1f", self.damage_est_last), "s")
            else
                self.status_text[2] = "estimation du temps critique..."
            end
        else
            self.status_text = { "CONFINEMENT ENDOMMAGE", "niveau de degats en baisse..." }
        end
    ...
    elseif is_active(self.alarms.ContainmentRadiation) then
        self.status_text[1] = "RAYONNEMENT DETECTE"

        if self.last_radiation >= const.EXTREME_RADIATION then
            self.status_text[2] = "niveau de radiation extremement eleve"
        elseif self.last_radiation >= const.SEVERE_RADIATION then
            self.status_text[2] = "niveau de radiation severement eleve"
        elseif self.last_radiation >= const.VERY_HIGH_RADIATION then
            self.status_text[2] = "niveau de radiation tres eleve"
        elseif self.last_radiation >= const.HIGH_RADIATION then
            self.status_text[2] = "niveau de radiation eleve"
        elseif self.last_radiation >= const.HAZARD_RADIATION then
            self.status_text[2] = "niveau de radiation dangereux"
        else
            self.status_text[2] = "niveau de radiation en hausse"
        end
    elseif is_active(self.alarms.ReactorOverTemp) then
        self.status_text = { "SURCHAUFFE DU COEUR", "temp. coeur cause des degats" }
    elseif is_active(self.alarms.ReactorWasteLeak) then
        self.status_text = { "FUITE DE DECHETS", "fuite radioactive detectee" }
    elseif is_active(self.alarms.ReactorHighTemp) then
        self.status_text = { "TEMP. COEUR ELEVEE", "temperature coeur elevee" }
    elseif is_active(self.alarms.ReactorHighWaste) then
        self.status_text = { "NIVEAU DE DECHETS ELEVE", "dechets en accumulation" }
    elseif is_active(self.alarms.TurbineTrip) then
        self.status_text = { "DEFAUT TURBINE", "blocage turbine detecte" }
    elseif is_active(self.alarms.RCSTransient) then
        self.status_text = { "TRANSITOIRE RCS", "verifiez le circuit caloporteur" }
    ...
    elseif self.em_cool_opened then
        self.status_text = { "REFROID. URGENCE OUVERT", "reinit RPS pour fermer vanne" }
    -- connection dependent states
    elseif self.plc_i ~= nil then
        local plc_db = self.plc_i.get_db()
        if plc_db.mek_status.status then
            self.status_text[1] = "ACTIF"

            if annunc.ReactorHighDeltaT then
                self.status_text[2] = "temperature coeur en hausse"
            elseif annunc.ReactorTempHigh then
                self.status_text[2] = "temp. coeur elevee, systeme nominal"
            elseif annunc.FuelInputRateLow then
                self.status_text[2] = "debit entree carburant insuffisant"
            elseif annunc.WasteLineOcclusion then
                self.status_text[2] = "debit sortie dechets insuffisant"
            elseif (util.time_ms() - self.last_rate_change_ms) <= FLOW_STABILITY_DELAY_MS then
                self.status_text[2] = "attente stabilite debit caloporteur"
            elseif not self.turbine_flow_stable then
                self.status_text[2] = "attente stabilite debit turbine"
            else
                self.status_text[2] = "systeme nominal"
            end
        ...
        elseif annunc.RadiationWarning then
            ...
            self.status_text = { "RAYONNEMENT DETECTE", "niveau de radiation en hausse" }
        else
            self.status_text[1] = "INACTIF"

            local temp = plc_db.mek_status.temp
            if temp < 350 then
                self.status_text[2] = "coeur froid"
            elseif temp < 600 then
                self.status_text[2] = "coeur tiede"
            else
                self.status_text[2] = "coeur chaud"
            end
        end
    elseif annunc.RadiationWarning then
        ...
        self.status_text = { "REACTEUR HORS LIGNE", "en attente de connexion..." }
    end
end
