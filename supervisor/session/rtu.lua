...
local svrs_fusion   = require("supervisor.session.rtu.fusion")
        ...
        for i = 1, #self.advert do
            ...
            if u_type == false then
                ...
                elseif unit_advert.reactor > 0 then
                    ...
                    elseif u_type == RTU_UNIT_TYPE.FUSION then
                        -- fusion reactor
                        unit = svrs_fusion.new(id, i, unit_advert, self.modbus_q)
                        if type(unit) ~= "nil" then target_unit.add_fusion(unit) end
