...
local function _update_dev_dbg()
    ...
    for u = 1, #units do
        ...
        for i = 1, #u_conns.fusion do
            if (rtu_conns.fusion[i] or false) ~= u_conns.fusion[i] then
                report(u_conns.fusion[i], util.c("unit ", u, "'s fusion reactor"))
                u_conns.fusion[i] = rtu_conns.fusion[i] or false
            end
        end
    end
end
...
function svsessions.check_rtu_id(unit, list, max)
    ...
    if fail_code ~= RTU_ID_FAIL.OK and fail_code ~= RTU_ID_FAIL.MAX_DEVICES then
        ...
        if r_id == 0 then
            ...
            elseif type == RTU_TYPES.FUSION then
                msg = msg .. "fusion reactor"
            elseif type == RTU_TYPES.ENV_DETECTOR then
                msg = util.c(msg, "#", idx, " env. detector")
            else
                msg = msg .. " ? (error)"
...
function svsessions.init(fp_ok, config, facility)
    ...
    for i = 1, config.UnitCount do
        ...
        local conns = { boilers = {}, turbines = {}, tanks = {}, fusion = {} }   ---@type unit_connections
