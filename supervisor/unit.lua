...
---@diagnostic disable-next-line: missing-fields
        ...
        status_text = { "INCONNU", "en attente de connexion..." },
    ...
    self.rtu_list = { self.redstone, self.boilers, self.turbines, self.tanks, self.fusion, self.snas, self.envd }
    ...
    -- link a fusion reactor RTU session
    ---@param fusion unit_session
    ---@return boolean linked fusion reactor accepted (max 1)
    function public.add_fusion(fusion)
        local fail_code, fail_str = svsessions.check_rtu_id(fusion, self.fusion, 1)
        local ok = fail_code == RTU_ID_FAIL.OK

        if ok then
            table.insert(self.fusion, fusion)
            log.debug(util.c(log_tag, "linked fusion reactor [", fusion.get_unit_id(), "@", fusion.get_session_id(), "]"))
        else
            log.warning(util.c(log_tag, "rejected fusion reactor linking due to failure code ", fail_code, " (", fail_str, ")"))
        end
    ...
    function public.update()
        ...
        -- check fusion reactors formed/faulted
        for i = 1, #self.fusion do
            local sess = self.fusion[i]
            local fusion = sess.get_db()
            if sess.is_faulted() or not fusion.formed then
                self.db.control.degraded = true
            end
        end
    ...
    function public.get_build(filter)
        ...
        if all or (filter == RTU_UNIT_TYPE.FUSION) then
            build.fusion = {}
            for i = 1, #self.fusion do
                local fusion = self.fusion[i]
                build.fusion[fusion.get_device_idx()] = { fusion.get_db().formed, fusion.get_db().build }
            end
        end
    ...
    function public.check_rtu_conns()
        ...
        conns.fusion = {}
        for i = 1, #self.fusion do
            conns.fusion[self.fusion[i].get_device_idx()] = true
        end
    ...
    function public.get_rtu_statuses()
        ...
        -- status of fusion reactors
        status.fusion = {}
        for i = 1, #self.fusion do
            local fusion = self.fusion[i]
            local db = fusion.get_db()
            status.fusion[fusion.get_device_idx()] = { fusion.is_faulted(), db.formed, db.state, db.tanks }
        end
